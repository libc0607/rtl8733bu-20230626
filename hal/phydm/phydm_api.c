...
phydm_api_shift_txagc(void *dm_void, u32 pwr_offset, enum rf_path path,
	...
	if (dm->support_ic_type & (ODM_RTL8733B)) {
		...
		#if (RTL8733B_SUPPORT)
		ret = config_phydm_write_txagc_ref_8733b(dm, (u8)txagc_cck,
							 path, PDM_CCK);
		ret &= config_phydm_write_txagc_ref_8733b(dm, (u8)txagc_ofdm,
							 path, PDM_OFDM);
		#endif
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "%s: path-%d txagc_cck_ref=%x txagc_ofdm_ref=0x%x\n",
		    ...
		    u8 rate, boolean is_single_rate)
{
	...
	boolean ret = false;
	     ...
	     RTL8814B_SUPPORT || RTL8197G_SUPPORT || RTL8733B_SUPPORT)
	u8 base = 0;
	u8 txagc_tmp = 0;
	s8 pw_by_rate_tmp = 0;
	s8 pw_by_rate_new = 0;
	...
	if (dm->support_ic_type & ODM_RTL8733B) {
		if (rate < 0x4)
			txagc_tmp = config_phydm_read_txagc_8733b(dm, path,
								  rate,
								  PDM_CCK);
		else
			txagc_tmp = config_phydm_read_txagc_8733b(dm, path,
		...
		pw_by_rate_tmp = config_phydm_read_txagc_diff_8733b(dm, rate);
		base = txagc_tmp - pw_by_rate_tmp;
		base = base & 0x7f;
		if (DIFF_2((pwr_idx & 0x7f), base) > 63 || pwr_idx > 127)
			return false;
		...
		pw_by_rate_new = (s8)(pwr_idx - base);
		ret = phydm_write_txagc_1byte_8733b(dm, pw_by_rate_new, rate);
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "%s: path-%d rate_idx=%x base=0x%x new_diff=0x%x\n",
			  __func__, path, rate, base, pw_by_rate_new);
	}
...
/*@jj add 20170822*/
	...
	if (dm->support_ic_type & ODM_RTL8733B) {
		if (hw_rate < 0x4) {
			ret = config_phydm_read_txagc_8733b(dm, path, hw_rate,
							    PDM_CCK);
		} else {
			ret = config_phydm_read_txagc_8733b(dm, path, hw_rate,
							    PDM_OFDM);
...
#if (RTL8733B_SUPPORT)
	case ODM_RTL8733B:
		ret = config_phydm_switch_channel_bw_8733b(dm, ch, pri_ch, bw);
	break;
#endif
		   ...
		   enum bb_path tx_path_ctrl)
{
	...
	#if (RTL8733B_SUPPORT)
	case ODM_RTL8733B:
		ret = config_phydm_trx_mode_8733b(dm, tx_path, rx_path,
						  tx_path_ctrl);
		break;
	#endif
