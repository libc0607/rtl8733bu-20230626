...
void phydm_get_thermal_trim_offset_8733b(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct odm_power_trim_data *power_trim_info = &dm->power_trim_data;
	u8 pg_therm = 0xff;
...
#if 1
	odm_efuse_one_byte_read(dm, PPG_THERMAL_OFFSET_8733B, &pg_therm, false);
	...
	RF_DBG(dm, DBG_RF_MP, "[kfree] 8733B thermal trim flag:0x%02x\n",
	       power_trim_info->flag);

	if (power_trim_info->flag & KFREE_FLAG_THERMAL_K_ON)
		RF_DBG(dm, DBG_RF_MP, "[kfree] 8733B thermal:%d\n",
		       power_trim_info->thermal);	
#endif
}

void phydm_get_set_power_trim_offset_8733b(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct odm_power_trim_data *power_trim_info = &dm->power_trim_data;	
	u8 pg_power = 0xff, i, j;
...
void phydm_get_tssi_trim_offset_8733b(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct odm_power_trim_data *power_trim_info = &dm->power_trim_data;
	u8 i, j, k;
	u8 pg_power[16] = {0xff, 0xff, 0xff, 0xff,
			   ...
			   0xff, 0xff, 0xff, 0xff};
#if 1
	odm_efuse_one_byte_read(dm, PPG_S0_CH3_TSSIDE_8733B, &pg_power[0], false);
	odm_efuse_one_byte_read(dm, PPG_S1_CH3_TSSIDE_8733B, &pg_power[1], false);
	odm_efuse_one_byte_read(dm, PPG_S0_CH11_TSSIDE_8733B, &pg_power[2], false);
	odm_efuse_one_byte_read(dm, PPG_S1_CH11_TSSIDE_8733B, &pg_power[3], false);
	odm_efuse_one_byte_read(dm, PPG_S0_CH42_TSSIDE_8733B, &pg_power[4], false);
	odm_efuse_one_byte_read(dm, PPG_S0_CH58_TSSIDE_8733B, &pg_power[6], false);
	odm_efuse_one_byte_read(dm, PPG_S0_CH110_TSSIDE_8733B, &pg_power[8], false);
	odm_efuse_one_byte_read(dm, PPG_S0_CH134_TSSIDE_8733B, &pg_power[10], false);
	odm_efuse_one_byte_read(dm, PPG_S0_CH159_TSSIDE_8733B, &pg_power[12], false);
	odm_efuse_one_byte_read(dm, PPG_S0_CH171_TSSIDE_8733B, &pg_power[14], false);
	...
	if (j == 16) {
		...
		RF_DBG(dm, DBG_RF_MP, "[kfree] 8733B tssi trim no PG\n");
	} else {
		power_trim_info->tssi_trim[0][0] = (s8)pg_power[0];
		power_trim_info->tssi_trim[0][1] = (s8)pg_power[1];
		...
		if (power_trim_info->flag & TSSI_TRIM_FLAG_ON) {
			for (i = 0; i < 8; i++) { //KFREE_BAND_NUM
				for (j = 0; j < 2; j++) {
					RF_DBG(dm, DBG_RF_MP,
					       "[kfree] 8733B tssi_trim[%d][%d]=0x%X\n",
					       i, j, power_trim_info->tssi_trim[i][j]);
				}
			}
		}
...
s8 phydm_get_tssi_trim_de_8733b(void *dm_void, u8 path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct odm_power_trim_data *power_trim_info = &dm->power_trim_data;
	u8 channel = *dm->channel, group = 0;
...
s8 phydm_get_tssi_trim_de(void *dm_void, u8 path)
{
	...
	if (dm->support_ic_type & ODM_RTL8822C)
		...
		return phydm_get_tssi_trim_de_8733b(dm, path);
	else
		return 0;
}
